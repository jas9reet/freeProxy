<?php
/*
NOTE: FreeProxy IS NO LONGER MAINTAINED AS OF APRIL 26th, 2020.
IF YOU USE IT, YOU DO SO ENTIRELY AT YOUR OWN RISK.
More information is available at <https://github.com/joshdick/FreeProxy>.
*/

/*
FreeProxy - A simple PHP web proxy. <https://github.com/joshdick/FreeProxy>
Written and maintained by Joshua Dick <http://joshdick.net>.
FreeProxy is licensed under the GNU GPL v3 <https://www.gnu.org/licenses/gpl-3.0.html>.
*/
...
//Set to false to allow sites on the local network (where FreeProxy is running) to be proxied.
$disallowLocal = true;
...
//Start/default URL that that will be proxied when FreeProxy is first loaded in a browser/accessed directly with no URL to proxy.
//If empty, FreeProxy will show its own landing page.
$startURL = "";

//When no $startURL is configured above, FreeProxy will show its own landing page with a URL form field
//and the configured example URL. The example URL appears in the instructional text on the FreeProxy landing page,
...
if (version_compare(PHP_VERSION, "5.4.7", "<")) {
  die("FreeProxy requires PHP version 5.4.7 or later.");
}

$requiredExtensions = ["curl", "mbstring", "xml"];
foreach($requiredExtensions as $requiredExtension) {
  if (!extension_loaded($requiredExtension)) {
    die("FreeProxy requires PHP's \"" . $requiredExtension . "\" extension. Please install/enable it on your server and try again.");
  }
}
...
function makeRequest($url) {
  ...
  if (empty($user_agent)) {
    $user_agent = "Mozilla/5.0 (compatible; FreeProxy)";
  }
  $ch = curl_init();
  ...
  switch ($_SERVER["REQUEST_METHOD"]) {
    case "POST":
      ...
      //If the FreeProxyFormAction field appears in the POST data, remove it so the destination server doesn't receive it.
      $postData = [];
      parse_str(file_get_contents("php://input"), $postData);
      if (isset($postData["FreeProxyFormAction"])) {
        unset($postData["FreeProxyFormAction"]);
      }
...
if (isset($_POST["FreeProxyFormAction"])) {
  $url = $_POST["FreeProxyFormAction"];
  unset($_POST["FreeProxyFormAction"]);
} else {
  $queryParams = [];
  parse_str($_SERVER["QUERY_STRING"], $queryParams);
  //If the FreeProxyFormAction field appears in the query string, make $url start with its value, and rebuild the the query string without it.
  if (isset($queryParams["FreeProxyFormAction"])) {
    $formAction = $queryParams["FreeProxyFormAction"];
    unset($queryParams["FreeProxyFormAction"]);
...
if (empty($url)) {
    if (empty($startURL)) {
      die("<html><head><title>FreeProxy</title>
        <meta charset=\"utf-8\" />
        <meta http-equiv=\"Content-type\" content=\"text/html; charset=utf-8\" />
        <meta name=\"viewport\" content=\"width=device-width, initial-scale=1\" />
          <link rel=\"stylesheet\" href=\"https://maxcdn.bootstrapcdn.com/bootstrap/4.0.0/css/bootstrap.min.css\" integrity=\"sha384-Gn5384xqQ1aoWXA+058RXPxPg6fy4IWvTNh0E263XmFcJlSAwiGgFAW/dAiS6JXm\" crossorigin=\"anonymous\">
          
          <link rel=\"icon\" href=\"ass3ts/img/fav.png\" type=\"image/gif\" >    
        <style>
    ...
    </style> </head><body><div class=\"divBox\" class=\"body\"><center><h1>FreeProxy</h1><br><form onsubmit=\"if (document.getElementById('site').value) { window.location.href='" . PROXY_PREFIX . "' + document.getElementById('site').value; return false; } else { window.location.href='" . PROXY_PREFIX . $landingExampleURL . "'; return false; }\" autocomplete=\"off\"><input id=\"site\" type=\"url\" placeholder=\"Enter URL \" /><input type=\"submit\" value=\"Surf!\" /></form></div></center></body></html>");
    } else {
      $url = $startURL;
    }
    ...
    //See https://github.com/joshdick/FreeProxy/pull/14
    $pos = strpos($url, ":/");
...
if (empty($scheme)) {
  if (strpos($url, "//") === 0) {
    ...
    die('Error: Detected a "' . $scheme . '" URL. FreeProxy exclusively supports http[s] URLs.');
}
...
if (stripos($contentType, "text/html") !== false) {
  ...
  foreach($xpath->query("//form") as $form) {
    ...
    $actionInput->appendXML('<input type="hidden" name="FreeProxyFormAction" value="' . htmlspecialchars($action) . '" />');
  ...
  echo "<!-- Proxified page constructed by FreeProxy -->\n" . $doc->saveHTML();
